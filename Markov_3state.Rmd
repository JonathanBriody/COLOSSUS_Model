...
# (I'm leaving this here for now, but ultimately I decided against doing any of this, given a comment made by Joshua in email).
...
# I very much decided against this.
...
# As will the 50 minute mark of: Cost-Effectiveness and Decision Modeling using R Workshop _ DARTH - Live Session August 25th WITH CHAT
...
# The above also describes how to convert probabilities for different time scales, i.e., convert a probability for 5 years to 1 year, etc., and how to convert data that exists as a rate to a probability for use in a Markov model.
 ...
 # The below reflects the probability of going to death in the second line treatment, and I make the assumption that everyone gets the same second line treatment and give them the same probability under exp and under SoC to go from the second line therapy into dead.
...
c_PFS_Folfox <-c(307.81, 1276.66, 285.54) 
c_PFS_Bevacizumab <-c(2580.38, 2952.24, 1325.87)  
c_OS_Folfiri <-c(326.02, 1309.64, 139.58)  
administration_cost <- c(365.00, 1794.40, 314.94)  
...
# I don't use [] here because I am still creating costs, which means I am still creating a concatanated vector which I will draw from later in building my graphs and ICERs.
...
# Error in m_M_SoC %*% c(c_F_SoC, c_P, c_D) : non-conformable arguments

# I 

m_M_SoC <- c(m_M_SoC, m_M_SoC, m_M_SoC)

concat_c_F_SoC <- m_M_SoC %*% c(c_F_SoC)
concat_c_P <- m_M_SoC %*% c(c_P)
concat_c_D <- m_M_SoC * (c_D)
# c_D isnt a vector because it's just 0 (the cost of treating the dead is 0 because they are dead, so we use * here instead of %*% or else we get the error: Error in m_M_SoC %*% (c_D) : non-conformable arguments)

v_tc_SoC <- c(concat_c_F_SoC, concat_c_P, concat_c_D)
...
c_F_Exp_Ireland <- m_M_Exp %*% 3640.802 
c_F_Exp_Germany <- m_M_Exp %*% 6410.912
c_F_Exp_Spain <- m_M_Exp %*% 2313.962


concat_c_P <- c(c_F_Exp_Ireland, c_F_Exp_Germany, c_F_Exp_Spain)


concat_c_P <- m_M_Exp %*% c(c_P)
concat_c_D <- m_M_Exp * c(c_D)
# c_D isnt a vector because it's just 0 (the cost of treating the dead is 0 because they are dead, so we use * here instead of %*% or else we get the error: Error in m_M_SoC %*% (c_D) : non-conformable arguments)


v_tc_Exp <- c(concat_c_F_Exp, concat_c_P, concat_c_D)

v_tc_Exp

# Now I should have a concatanated vector made from v_tc_soc, and should be able to use v_tc_soc[1] to find the v_tc_soc for Ireland, [2] for Germany, [3] for Spain, and so on, so I need to start searching for this and any other costs, and putting [] infront of it in the model.
...
# To avoid getting the following error: Error in t(v_tc_SoC) %*% v_dwc : non-conformable arguments, because of the concatanated v_tc_SoC and v_tc_Exp
# I concatanated v_dwc

concat_v_dwc <- c(v_dwc, v_dwc, v_dwc)


tc_d_SoC <-  t(v_tc_SoC) %*% concat_v_dwc 
tc_d_Exp <-  t(v_tc_Exp) %*% concat_v_dwc
