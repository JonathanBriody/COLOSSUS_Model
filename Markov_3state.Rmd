...
packages <- c("pacman", "utils", "stats", "fitdistrplus", "flexsurv", "MASS", "dplyr", "devtools", "scales", "ellipse", "ggplot2", "lazyeval", "igraph", "ggraph", "reshape2", "knitr", "stringr", "diagram", "survival", "lubridate", "ggsurvfit", "gtsummary", "tidycmprsk", "magrittr", "dplyr", "diffr", "gdata", "gridGraphics", "sjPlot", "stargazer", "tibble", "here", "collapse")
...
# # 1. Cost of treatment in this country
# 
# # I received my costs as cost's per month, so I update these to be fortnightly mobths as below:
# 
...
write.csv(df_cea, file = paste0("Baseline_ICER_For_", country_name, ".csv"), row.names = FALSE)
...
# I label each variable in my dataset so that when I plot their distributions in the PSA and when I plot the tornado diagram the variable labels appear rather than the variable names, as the variable names are often things like P_OSD_SoC which is more confusing than a simple name describing what the variable is:

# Add the comment (label)

# Probabilities:

comment(P_OSD_SoC) <- "Probability Death Second Line Treatment - SoC"
comment(P_OSD_Exp) <- "Probability Death Second Line Treatment - Exp"
comment(p_FA1_STD) <- "Probability Leukopenia - SoC"
comment(p_FA2_STD) <- "Probability Diarrhea - SoC"
comment(p_FA3_STD) <- "Probability Nausea - SoC"
comment(p_FA1_EXPR) <- "Probability Leukopenia - Exp"
comment(p_FA2_EXPR) <- "Probability Diarrhea - Exp"
comment(p_FA3_EXPR) <- "Probability Nausea - Exp"

# Costs:

comment(administration_cost) <- "Administration Cost"
comment(c_PFS_Folfox) <- "Folfox Cost"
comment(c_PFS_Bevacizumab) <- "Bevacizumab Cost"
comment(c_OS_Folfiri) <- "Folfiri Cost"
comment(c_AE1) <- "Leukopenia Cost"
comment(c_AE2) <- "Diarrhea Cost"
comment(c_AE3) <- "Nausea Cost"
comment(subtyping_test_cost) <- "Subtyping Test Cost"

# Utilities:

comment(u_F) <- "Utility First Line Treatment"
comment(u_P) <- "Utility Second Line Treatment"
comment(u_D) <- "Utility Dead"

comment(AE1_DisUtil) <- "Leukopenia Disutility"
comment(AE2_DisUtil) <- "Diarrhea Disutility"
comment(AE3_DisUtil) <- "Nausea Disutility"

# Discounting:

comment(d_c) <- "Cost Discount Rate"
comment(d_e) <- "Utility Discount Rate"


# Survival Data: 
# 
# comment(m_coef_weibull_SoC) <- "First to Second Line Parametric Survival Distributions"
# 
# comment(m_coef_weibull_OS_SoC) <- "First Line to Death Parametric Survival Distributions"
# 
# 

# Hazard Ratios:

comment(HR_FP_Exp) <- "Hazard Ratio First to Second Line Under Experimental Treatment"

comment(HR_PD_Exp) <- "Hazard Ratio First Line to Death Under Experimental Treatment"
...
# You'll notice a lot of plotting below. This is not needed to run the PSA. I just create all these plots because it gives a visual representation of my 10,000 draws for each variable. I can see what values were being drawn for each variable, whether they were on average at the mean I started out with, and whether they fit the distribution I assigned to them, i.e., gamma, beta, etc., because I overlay the line from these distributions on the draws I made and see how similar they look to eachother. 
  
# Assign the variable I am looking at to "plotted_variable"
plotted_variable <- "m_coef_weibull_SoC",
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# Source the PSA plotting script - this then applys the plotting script to the variable in question and saves the resultant plot:
#source("plotting PSA Distributions_Weibull.R"),


# Assign the variable I am looking at to "plotted_variable"
plotted_variable <- "m_coef_weibull_OS_SoC",
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# Source the PSA plotting script - this then applys the plotting script to the variable in question and saves the resultant plot:
#source("plotting PSA Distributions_Weibull_OS.R"),
...
# Plotting
plotted_variable <- MEAN_HR_FP_Exp,
# I have to create variable_label separately below or else the code won't run as a string value is entered rather than a numeric.
variable_label <- "MEAN_HR_FP_Exp",
plotted_sd <- SD,
comment(MEAN_HR_FP_Exp) <- "Hazard Ratio First to Second Line Under Experimental Treatment",
variable_label <- comment(get(variable_label)),
#source("plotting PSA Distributions lognormal.R"),
...
# Plotting
plotted_variable <- MEAN_HR_PD_Exp,
variable_label <- "MEAN_HR_PD_Exp",
plotted_sd <- OS_SD,
comment(MEAN_HR_PD_Exp) <- "Hazard Ratio First Line to Death Under Experimental Treatment",
variable_label <- comment(get(variable_label)),
#source("plotting PSA Distributions lognormal.R"),
...
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# These two steps allow me to source the PSA plotting script over and over again for each variable I am looking at, because in that script I apply my code to plotted_variable and variable_label  - such that the above always updates these to the variables I am interested in.

# Source the PSA plotting script - this then applys the plotting script to the variable in question and saves the resultant plot:
#source("plotting PSA Distributions.R"),
...
# Assign the variable I am looking at to "plotted_variable"
plotted_variable <- "administration_cost",
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# These two steps allow me to source the PSA plotting script over and over again for each variable I am looking at, because in that script I apply my code to plotted_variable and variable_label  - such that the above always updates these to the variables I am interested in.

# Source the PSA plotting script - this then applys the plotting script to the variable in question and saves the resultant plot:
#source("plotting PSA Distributions_Gamma.R"),
...
# Assign the variable I am looking at to "plotted_variable"
plotted_variable <- "c_PFS_Bevacizumab",
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# These two steps allow me to source the PSA plotting script over and over again for each variable I am looking at, because in that script I apply my code to plotted_variable and variable_label  - such that the above always updates these to the variables I am interested in.
...
# Assign the variable I am looking at to "plotted_variable"
plotted_variable <- "subtyping_test_cost",
# Assign the label of the variable I am looking at to "variable_label"
variable_label <- comment(get(plotted_variable)),
# These two steps allow me to source the PSA plotting script over and over again for each variable I am looking at, because in that script I apply my code to plotted_variable and variable_label  - such that the above always updates these to the variables I am interested in.
...
# WHEN PLOTTING UTILITIES REMEMBER THAT THEY WILL BE SLIGHLTY SKEWED AS THEY CANNOT GO ANY HIGHER THAN 1 OR ANY LOWER THAN ZERO, SO WHEN TAKING RANDOM DRAWS WITH A MEAN THAT IS CLOSER TO 1 THAN 0, I.E., 0.68, YOU WILL HAVE A LARGER SELECTION OF VALUES BELOW THE MEAN THAT YOU CAN SAMPLE THAN VALUES ABOVE THE MEAN, I.E., THERE IS ONLY 0.32 VALUES ABOVE THE MEAN BUT THERE ARE 0.67 BELOW THE MEAN.
...
#source("Plotting PSA Distributions uniform.R"),
...
table_cea_PA
# Save the dataframe of the CEA table to a CSV file
write.csv(df_cea_PA, file = paste0("Probabilistic_ICER_For_", country_name, ".csv"), row.names = FALSE)
