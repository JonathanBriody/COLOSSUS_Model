...
It may just be creating an wtp for the NMB calculation in the function, but probably a good idea to ensure this is the case (i.e., change the n_wtp to see how it changes the nmb) and also ensure that I am happy with the NMB value currently under use (10,000 above).

```{r}

# Typically, it's very easy to get the tornado plot to work with ICERS rather than with NMB, you just change   outcomes         = c("NMB"), to   outcomes         = c("DSAICER"), and then change the function to include the following code: 
...
# Basically straight after you calculate the net monetary benefit for the notrnado diagram, 


# I was having some trouble getting my tornado plot to generate. I think I've tracked the issue down to ggplot, which the dampack tornado plot code uses. I suspect that this was because some of my values were very small, because the exact issue I was getting was and per the following webpage.

# If I can't figure out how to get a tornado diagram to work with ICER's rather than NMB, I could follow the approach here of doing a probabilistic analysis and then creating a tornado diagram on the probabilistic analysis. These examples explain the technicalities of how this is done in R but also from a theoretical viewpoint how this works:
