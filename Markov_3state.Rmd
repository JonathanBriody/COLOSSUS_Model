...
Costs, utilities and probabilities were varied within +/- 20% of baseline, per [@goldstein2014a; @goulart2011]....
[these are just guys who have done the 20% varying, if I find better online I can use that].

To define the ranges of the mortalities and progression risks for the sensitivity analysis, we first adjusted the PFS/OS curves \$S(t)\$ in the base case by a hazard ratio adjustment \$\\gamma\$, as \$[S(t)\^{\\gamma}]\$, and then reestimated the risks for every cycle based on the adjusted survival curves.
The adjustment hazard ratio was chosen to be 1 +/- 0.2 (1 indicates that the base case value was used).
...
The adjustment hazard ratio was 1 +/- 0.2, i.e., the base case plus or minus 20%.

[So, basically what I need to do here is get my survival curves from the parametric survival analysis for standard of care, and for the novel therapy post applying the hazard ratio on Bevacizumab (so, this may require converting back to a survival curve) and then have them as the mean, and the min as the mean multiplied by 0.20 and -0.20 and the max as the mean +0.20].
...
[- per: Goldstein, D. A., Chen, Q., Ayer, T., Howard, D. H., Lipscomb, J., Harvey, R. D., ... & Flowers, C. R.
(2014).
Cost effectiveness analysis of pharmacokinetically-guided 5-fluorouracil in FOLFOX chemotherapy for metastatic colorectal cancer.
Clinical colorectal cancer, 13(4), 219-225.]

dampack has the functionality to interface with a user-defined decision model in R to conduct a deterministic sensitivity analysis (DSA) on parameters of interest.
DSA is a method for assessing how the results of a decision analytic model change as a parameter of interest in varied over a specified range of values.
This includes both how model outcomes change (e.g. the expected cost of each strategy) as well as how the decision changes (e.g. which strategy is the most cost-effective) with the parameter of interest.

In a DSA, the model is run for each value of the parameter of interest over the specified range, while holding all other parameter values fixed.
If one parameter is varied, this is called a one-way DSA.
If two parameters are varied, it is called a two-way DSA.
We typically do not vary more than two parameters at a time, as the output becomes difficult to visualize and interpret.
A more comprehensive assessment of how model outptus depend on model parameters can be done through a probabilistic sensitivity analysis (PSA) (type vignette("psa_generation", package = "dampack") in the console after installing the dampack package to see a vignette describing how to use dampack for PSA).
...
df_params_OWSA <- data.frame(
  pars = c("c_F_Exp", "u_F", "HR_FP_Exp", "HR_FP_SoC"),   # names of the parameters to be changed
  min  = c(400,  Minimum_u_F, Minimum_HR_FP_Exp, Minimum_HR_FP_SoC),         # min parameter values
  max  = c(1200, Maximum_u_F, Maximum_HR_FP_Exp, Maximum_HR_FP_SoC)          # max parameter values
)
