...
[**I need to figure out:**]{.underline}
...
> In case, a patient suffered different multiple events (e.g., a myocardial infarction [MI] and a stroke), the model followed the minimum approach, recording the utility value of the health state with the lowest individual score, while still applying a disutility related to [adverse]{.underline} events.
...
owsa_opt_strat(owsa = OWSA_NMB, txtsize = 10, plot_const = FALSE)
...
# When calculating se from point estimates, remember that utility and probability values cannot be less than 0 or greater than 1, so don't calculate a min or max that is less than 0 or greater than 1.
...
# If I don't describe generating random draws when the probabilities don't have an SE above, I'm sure the Goldstein paper will let me figure out how to do this in the same way that I figured out how to do this for the utility beta distributions below.
  ...
  p_A1D_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_A2F_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_A2D_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_A3F_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_A3D_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_FD_SoC  = rbeta(n_runs, shape1 = , shape2 = ),
  p_FD_Exp  = rbeta(n_runs, shape1 = , shape2 = ),
  p_PD_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_PD_Exp = rbeta(n_runs, shape1 = , shape2 = ),
  p_FA1_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_FA2_SoC = rbeta(n_runs, shape1 = , shape2 = ),
  p_FA3_SoC = rbeta(n_runs, shape1 = , shape2 = ),
...
# Sharp, L., Tilson, L., Whyte, S., O'Ceilleachair, A., Walsh, C., Usher, C., ... & Comber, H. (2012). Cost-effectiveness of population-based screening for colorectal cancer: a comparison of guaiac-based faecal occult blood testing, faecal immunochemical testing and flexible sigmoidoscopy. British journal of cancer, 106(5), 805-816. https://www.nature.com/articles/bjc2011580.pdf

# There may also be helpful books here: C:\Users\Jonathan\Dropbox\PhD\HTA\Markov Modelling\books
...
# For his paper, I update the Maximum/Minimum part by taking the max as 50% bigger than the mean. When I do this, I get the exact same SE as he reports in the excel file, I know this because in his cells creating the alpha and beta values for utility on sheet 2 of his excel file, he includes 2 numbers, one is the mean reported on sheet 1 for the parameter whose utility he is taking random draws from, and then the other number is an EXACT match for the SE number I calculate, and must therefore be the SE number he calculates. Especially because he then applies a formula to this mean and SE to generate the alpha and the beta. You can see this in my model code below, where the mean and SE I report in my own analysis turn up in his alpha and beta formulas.
...
# RANGES NEXT: See if Koen's paper can lead you to ranges for the values he included, check ranges available in your other sources. Look at how I deal with ranges above, if SE is calculated the same way, I imagine ranges are analysed the same way too.

# Should I take the furthest range when there is a range present and it's not equal from max to point estimate and min to point estimate?
...
# Koen's excel file can help with this too.
...
# If I want to do the discount rate I can use the following: # "Density, distribution function, quantile function and random generation for the Gamma distribution with parameters alpha (or shape) and beta (or scale or 1/rate)." https://search.r-project.org/CRAN/refmans/Rlab/html/Gamma.html also saved here: C:\Users\Jonathan\OneDrive - Royal College of Surgeons in Ireland\COLOSSUS\Training Resources\Decision Modelling - Advanced Course\A2_Making Models Probabilistic\A2.1.2 Distributions for parameters\13.5 Uniform (Continuous Version) _ Analytics Using R.pdf
...
# So I could say, the uncertainty in this limited sample size is propogated through the model by applying a dirichlet and beta distribution to a fully probabilistic analysis of the data.
...
# Here's what I think is happening, the formula to generate the se is se <- ((Maximum) - (Mean))/2 but the 2 is really a t-quantile with n-1 degrees of freedom, but I used 2 as a simplification because "For large n the quantile approaches 2.0 (well, 1.959964... to be more precise; but using 2.0 is good enough)." I think that the authors of this study actually looked up the t-quantile and divided by that each time, rather than dividing by 2, i.e. "The t-quantile can be looked up for the level of confidence when the total sample size (n) and the number of coefficients in the model (k) are known. For instance, the t-quantile for 95% confidence, n=10 and k=2 is 2.3." per: researchgate.net/post/Formula_for_calculate_Standard_errorSE_from_Confidence_IntervalCI There is also information supporting this here: https://stats.stackexchange.com/questions/550293/how-to-calculate-standard-error-given-mean-and-confidence-interval-for-a-gamma-d
...
# Doing this, I can prove that the differences between my results on their tables, and their results, comes down to rounding.





# However, when I generate the SE from the values reported in the paper, I don't have as many decimal points for my SE, i.e., it rounds up to 0.05 from 0.04616056, as below:
...
# I've checked this for all the beta values in the Table.
