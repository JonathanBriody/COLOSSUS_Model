...
# # 1. Cost of treatment in this country
#
# # I received my costs as cost's per month, so I update these to be fortnightly costs as below:
#
...
survfit2(Surv(time, status) ~ 1, data = df_TTP) %>% ggsurvfit() +
  ...
  add_risktable()
#print(plot)
ggsave("Baseline_PFS_Curves.png", width = 8, height = 4, dpi=600)
#dev.off()
...
survfit2(Surv(time, status) ~ 1, data = df_TTD) %>% ggsurvfit() +
  ...
  add_risktable()
ggsave("Baseline_OS_Curves.png", width = 4, height = 4, dpi=600)
...
# It's really important that I do this before creating any parameters to be used in the model, or I will have the issue where the country name keeps getting updated to Spain or whatever country name parameter is loaded when I create this!


# 
# df_TTP_EXP <- read.csv(file='PFS_WITHBEV.csv', header=TRUE)
# save.image("C:/Users/Jonathan/OneDrive - Royal College of Surgeons in Ireland/COLOSSUS/R Code/GitHub/COLOSSUS_Model/df_TTP_EXP.RData")
# df_TTD_EXP <- read.csv(file='OS_WITHBEV.csv', header=TRUE)
# save.image("C:/Users/Jonathan/OneDrive - Royal College of Surgeons in Ireland/COLOSSUS/R Code/GitHub/COLOSSUS_Model/df_TTD_EXP.RData")
...
# Save the plot
ggsave("Baseline_PFS_Curves_BothEXPANDSOC.png", width = 8, height = 6, dpi = 600)
...
# Save the plot
ggsave("Baseline_OS_Curves_BothEXPANDSOC.png", width = 8, height = 4, dpi = 600)
...
# Open a PNG device
png("Markov_Cohort_Traces.png", width = 8, height = 6,  units = "in", res = 600)
...
ggsave(paste("Frontier_Markov_Model_", country_name[1], ".png", sep = ""), width = 8, height = 4, dpi=600)
...
# First I generate the tornado plot using the owsa_tornado function
tornado_plot <- owsa_tornado(owsa = DSAICER, txtsize = 11, ylim = c(100000, 400000))


# If I wanted to create the tornado plot with custom x-axis limits I could do this as below, per: https://rdrr.io/github/DARTH-git/dampack/man/owsa_tornado.html
...
# Then I modify the x-axis label to "ICER" using the labs function from ggplot2, however, bevause the tornado plot generated by owsa_tornado has a horizontal orientation it means the labels are swapped, that is, to change the label on the x-axis correctly, we need to update the y-axis label instead:
tornado_plot <- tornado_plot + labs(y = "ICER")

# I adjust the alignment of the axis labels using the theme function in ggplot2. To center-align the y-axis label ("ICER"), you can specify the axis.title.y element in the theme function and set the vjust parameter to 0.5.

# To add a dynamic title to the tornado plot that includes the country name, I use the ggtitle function from ggplot2. 

# Add a title to the plot using the country name
plot_title <- paste("Tornado Plot", country_name[1])

# I  center the title of the plot in ggplot2 by using the theme function to adjust the alignment. The plot.title element can be customized with the element_text function, where you can set the hjust parameter to 0.5 for center alignment.
# So, the theme function is used to customize the plot.title element.
# So, the element_text function with hjust = 0.5 centers the title horizontally.

# To center-align the y-axis label with the title, :

# We can adjust the alignment of the axis labels using the theme function in ggplot2. To center-align the y-axis label ("ICER"), we can specify the axis.title.y element in the theme function and set the vjust parameter to 0.5.
tornado_plot <- tornado_plot + ggtitle(plot_title) + theme(plot.title = element_text(hjust = 0.5), axis.title.y = element_text(vjust = 0.25))

# By adjusting the vjust parameter for axis.title.y, the y-axis label "ICER" is vertically centered. If we also want to adjust the horizontal alignment of the y-axis label, we can use hjust similarly.

# I save the modified plot to a file as before, but now add "plot = tornado_plot":
ggsave(paste("Tornado_Diagram_", country_name[1], ".png", sep = ""), plot = tornado_plot, width = 8, height = 4, dpi=600)
# Close the graphics device
while (!is.null(dev.list()))  dev.off()
#png(paste("Tornado_Diagram_", country_name[1], ".png", sep = ""))
#dev.off()

# The [1] in country_name[1] is used to ensure that only the first element of the country_name vector is used, assuming country_name might be a vector with multiple entries. However, if country_name is a single-element character vector, you can directly use country_name without the [1].
...
unlimited_tornado_plot <- owsa_tornado(owsa = DSAICER, txtsize = 11)

unlimited_tornado_plot <- unlimited_tornado_plot + labs(y = "ICER")

plot_title <- paste("Tornado Plot", country_name[1])

unlimited_tornado_plot <- unlimited_tornado_plot + ggtitle(plot_title) + theme(plot.title = element_text(hjust = 0.5), axis.title.y = element_text(vjust = 0.25))

ggsave(paste("Unlimited_Tornado_Diagram_", country_name[1], ".png", sep = ""), plot = unlimited_tornado_plot, width = 8, height = 4, dpi=600)
while (!is.null(dev.list()))  dev.off()

# The dpi argument in the ggsave function has been set to 600 to increase the resolution of the saved images. Higher values of dpi (dots per inch) result in higher resolution images.
# This setting is applied to both the limited and unlimited versions of the tornado plot.
# By adjusting the dpi setting, you ensure that the output images are of higher resolution, making them suitable for high-quality prints or detailed analysis.
...
ggsave(paste("CE_Scatter_Plot_", country_name[1], ".png", sep = ""), plot = ce_scatter_plot, width = 8, height = 4, dpi = 600)
...
ggsave(paste("Unlimited_CE_Scatter_Plot_", country_name[1], ".png", sep = ""), plot = unlimited_ce_scatter_plot, width = 8, height = 4, dpi = 600)
while (!is.null(dev.list())) dev.off()
...
# Save the plot to a file
ggsave(paste("CEAC_", country_name[1], ".png", sep = ""), plot = ceac_plot, width = 8, height = 4, dpi = 600)
