---
title: 'Bevacizumab for Metastatic Colorectal Cancer with Chromosomal Instability: An Cost-Effectiveness Analysis across the COLOSSUS Partner Countries'
...
Recently, copy number load has identified a subtype of patients with improved outcomes following Bevacizumab combination therapy.
...
**Materials and Methods:** We developed a three-state Markov model with fortnightly cycles to estimate the incremental cost-effectiveness ratio (ICER) of Bevacizumab plus FOLFOX (5-FU, leucovorin, and oxaliplatin) versus FOLFOX alone.
...
The perspective was that of a healthcare payer in the countries of the COLOSSUS translational study (Germany, Ireland and Spain).
To test the robustness of the results, parameter values were varied individually to assess the model's sensitivity to individual parameters, and simultaneously, sampling multiple sets of parameter values from a priori--defined probability distributions to represent uncertainty in model parameters.
...
Cost-effectiveness acceptability curves from the probabilistic sensitivity analysis showed that adding Bevacizumab to therapy was only cost-effective when willingness to pay thresholds were multiple times larger than currently used cut-offs.
...
This may correspond to a limited overall benefit of Bevacizumab in the clinic [\@potti2010].
...
ANGIOPREDICT patients treated demonstrated significantly improved PFS (HR = 0.68, P = 0.00249, CI 0.53--0.87) and OS (HR = 0.65, P = 0.00227, CI 0.49−0.86) after adding Bevacizumab to treatment compared to chemotherapy alone.
...
Consequently, we conducted a health economic analysis of treatment in this emergent high chromosomal instability group.
...
Variability in pricing, healthcare costs, willingness to pay thresholds and multiple international payer perspectives are such that cost-effectiveness may be expected to differ across countries.
The COLOSSUS consortium comprises many of the same investigators for the ANGIOPREDICT study, and .
Consequently, we can establish the cost-effectiveness of bevacizumab in mCRC in the consortium countries of Ireland, Germany and Spain.
...
Per [@goldstein2017] probability of transitions, adverse events, and quality of life under treatment were assumed to be generalizable, consequently shared values were applied to all countries under study.
Country-specific cost data was gathered via direct survey of consortium hospitals in the participating countries.
