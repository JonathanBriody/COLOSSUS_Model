...
\# Minimum_p\_FP_SoC \<- p_FP_SoC - 0.20\*p_FP_SoC

\# Maximum_p\_FP_SoC \<- p_FP_SoC + 0.20\*p_FP_SoC

\# Minimum_p\_FP_Exp \<- p_FP_Exp - 0.20\*p_FP_Exp

\# Maximum_p\_FP_Exp \<- p_FP_Exp + 0.20\*p_FP_Exp
...
Finally, we conduct a two-way uncertainty analysis to investigate the changes in the outcome when varying two parameters simultaneously.
As informed by [@briggs2012] we conduct a two-way sensitivity analysis for parameters expected to have a logical covariance.
For these parameters of interest - the event rate in the control intervention and the hazard ratio with the new treatment - we examine how the expected outcome (here NMB) changes as both parameters change.
[These are expected to have a relationship because the hazard ratio for the experimental is multiplied by the rate under soc]
...
df_params_TWSA <- data.frame(pars = c("p_FP_SoC", "HR_FP_Exp"),
                             min  = c(Minimum_p_FP_SoC, Minimum_HR_FP_Exp),  # min parameter values
                             max  = c(Maximum_p_FP_SoC, Maximum_HR_FP_Exp) # max parameter values
)
...
This made provide some useful guides on the PSA <https://rdrr.io/github/DARTH-git/dampack/api/>
...
```{r}
 ...
 DOES MVNORM MEAN I DONT NEED TO INCLUDE HR_FP_SoC ANY LONGER? 
# According to:
#
