...
t_cycle <- 30      # cycle length of 4 weeks (in [[days]] - this is assuming the survival curves I am digitising will be in [[days]] if they are in another period I will have to represent my cycle length in that period instead).                                  
n_cycle        <- 67                            
# We set the number of cycles to 67 to reflect 2,000 days from the Angiopredict study (5 Years, 5 Months, 3 Weeks, 1 Day) broken down into fortnightly cycles
...
for(i in 1:n_cycle) {
  p_PD_SoC[i] <- 1 - NU_S_PD_SoC[i+1] / NU_S_PD_SoC[i]
...
p_A1_D_STD    <- 0.10    # probability of dying when in adverse event 1 under SOC

p_FA2_STD     <- 0.310   # probability of adverse event 2 when progression-free under SOC
p_A2_D_STD    <- 0.10    # probability of dying when in adverse event 2 under SOC

p_FA3_STD     <- 0.310   # probability of adverse event 3 when progression-free under SOC
p_A3_D_STD    <- 0.10    # probability of dying when in adverse event 3 under SOC


p_FA1_EXPR     <- 0.070   # probability of adverse event 1 when progression-free under EXPR
p_A1_D_EXPR    <- 0.10    # probability of dying when in adverse event 1 under EXPR

p_FA2_EXPR     <- 0.110   # probability of adverse event 2 when progression-free under EXPR
p_A2_D_EXPR    <- 0.10    # probability of dying when in adverse event 2 under EXPR

p_FA3_EXPR     <- 0.070   # probability of adverse event 3 when progression-free under EXPR
p_A3_D_EXPR    <- 0.10    # probability of dying when in adverse event 3 under EXPR
