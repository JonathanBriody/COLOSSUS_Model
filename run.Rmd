...
# For building the Table in the paper, I create country specific versions of the parameters to be included in the Table:
...
# I don't save these two values using "keep" as they'll be leftover from running the code, and should always come out the same as they are from the ANGIOPREDICT data on peoples survival and not dependent on the country we are studying, so the last country studied should have them saved on the final run of the model.
